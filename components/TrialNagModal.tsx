...
import { X, Zap, Shield, TrendingUp, ArrowRight, Sparkles, CheckCircle } from 'lucide-react';
import { redirectToCheckout } from '../services/stripeService';
import { logSubscriptionEvent } from '../services/subscriptionService';
import { getCurrentUser } from '../services/authService';

interface TrialNagModalProps {
  onClose: () => void;
  onUpgrade: () => void;
  reportsRemaining: number;
}

const TrialNagModal: React.FC<TrialNagModalProps> = ({ onClose, onUpgrade, reportsRemaining }) => {
  ...
  const handleDismiss = () => {
    ...
    localStorage.setItem('trial_nag_dismissed', Date.now().toString());
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-black/60 dark:bg-black/80 backdrop-blur-md animate-in fade-in overflow-y-auto">
      <div className="relative w-full max-w-5xl bg-white dark:bg-[#111827] rounded-2xl sm:rounded-3xl shadow-2xl dark:shadow-[0_0_50px_rgba(87,122,255,0.4)] overflow-y-auto max-h-[95vh] my-auto animate-in slide-in-from-bottom-4 duration-500 border-2 border-[#D5DDFF] dark:border-[#374151]">
        {/* Close button */}
        <button
          onClick={handleDismiss}
          className="absolute top-6 right-6 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-[#1a1f2e] transition-colors z-10 group"
          aria-label="Close"
        >
          <X className="w-6 h-6 text-[#595657] dark:text-[#9ca3af] group-hover:text-[#051A53] dark:group-hover:text-[#f3f4f6]" />
        </button>
        ...
        <div className="p-4 sm:p-6 md:p-8 lg:p-12">
          ...
          {/* Error message */}
          {error && (
            <div className="mb-6 p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800/50">
